...
struct nhan_vat// bao gom nguoi choi va chuong ngai vat
{
    ...
    void animation(int n)
    {
        if(status == 0)
        {
            ...
            gSpriteSheetTexture[index].render(gRenderer ,x, y, &current_clip);
...
bool loadMedia()
{
    ...
    if(!gSpriteSheetTexture[0].loadFromFile("dino.png", gRenderer))
    {
    ...
    if( !gSpriteSheetTexture[1].loadFromFile( "bat.png",gRenderer ) )
    {
        printf( "Failed to load sprite sheet texture!\n" );
        success = false;
    }
    ...
    if( !gSpriteSheetTexture[2].loadFromFile( "cartus.png", gRenderer ) )
    {
        printf( "Failed to load sprite sheet texture!\n" );
        success = false;
    }
    ...
    if( !gSpriteSheetTexture[3].loadFromFile( "angry_cartus.png" , gRenderer) )
    {
        printf( "Failed to load sprite sheet texture!\n" );
        success = false;
    }
    ...
    if( !gSpriteSheetTexture[4].loadFromFile( "some_cartus.png" , gRenderer) )
    {
        printf( "Failed to load sprite sheet texture!\n" );
        success = false;
    }
    ...
    if( !gBackgroundTexture.loadFromFile( "night.jpg" ,gRenderer) )
    {
        printf( "Failed to load background texture image!\n" );
        success = false;
    }
    if( !gmenu.loadFromFile( "menu.jpg" ,gRenderer) )
    {
        printf( "Failed to load background texture image!\n" );
        success = false;
    }
    if( !ginstruction.loadFromFile( "mouse_instruction.jpg",gRenderer ) )
    {
        printf( "Failed to load background texture image!\n" );
        success = false;
    }
    if( !play_again.loadFromFile( "play_again.jpg" ,gRenderer) )
    {
        printf( "Failed to load background texture image!\n" );
        success = false;
    }
    if( !high_score.loadFromFile( "high.jpg" ,gRenderer) )
    {
        printf( "Failed to load background texture image!\n" );
        success = false;
    }
    if(!gBackgroundPause.loadFromFile("background_pause.jpg",gRenderer))
    {
        printf( "Failed to load background texture image!\n" );
        success = false;
    }
...
{
    if(score > _high_score[10])
    {
        if(score > _high_score[0])
        {
            _high_score.insert(_high_score.begin(), score);
            _high_score.erase(_high_score.begin() + 10);
        }
        else
        {
            int i = 0;
            for(i = 0; i < 9; i++)
            {
                if(_high_score[i] > score && score > _high_score[i+1])
                break;
            }
                  ...
                  bool &high_sc,SDL_Event &e, int &mou_x,int &mou_y  )
{
    ...
    while( quit==false && ( is_instruction == true || is_menu== true || is_play == true || high_sc == true )  )
    {
        if( is_menu== true )
        {
            gmenu.render(gRenderer,0,0,NULL);
            SDL_RenderPresent(gRenderer);
        }
        if( is_instruction==true)
        {
            ginstruction.render(gRenderer,0,0,NULL);
            SDL_RenderPresent(gRenderer);
        }
        if( high_sc == true )
        {
            high_score.render(gRenderer,0,0,NULL);
            HIGH_SCORE_0.loadFromRenderedText(trans(_high_score[0]),gFont, RED_COLOR, gRenderer);
            HIGH_SCORE_0.render(gRenderer,855, 170,NULL);
            HIGH_SCORE_1.loadFromRenderedText(trans(_high_score[1]),gFont, RED_COLOR, gRenderer);
            HIGH_SCORE_1.render(gRenderer,855, 205,NULL);
            HIGH_SCORE_2.loadFromRenderedText(trans(_high_score[2]),gFont, RED_COLOR, gRenderer);
            HIGH_SCORE_2.render(gRenderer,855, 240, NULL);
            HIGH_SCORE_3.loadFromRenderedText(trans(_high_score[3]),gFont, RED_COLOR, gRenderer);
            HIGH_SCORE_3.render(gRenderer,855, 270,NULL);
            HIGH_SCORE_4.loadFromRenderedText(trans(_high_score[4]),gFont, RED_COLOR, gRenderer);
            HIGH_SCORE_4.render(gRenderer,855, 305,NULL);
            HIGH_SCORE_5.loadFromRenderedText(trans(_high_score[5]),gFont, RED_COLOR, gRenderer);
            HIGH_SCORE_5.render(gRenderer,855, 340,NULL);
            HIGH_SCORE_6.loadFromRenderedText(trans(_high_score[6]),gFont, RED_COLOR, gRenderer);
            HIGH_SCORE_6.render(gRenderer,855, 370,NULL);
            HIGH_SCORE_7.loadFromRenderedText(trans(_high_score[7]),gFont, RED_COLOR, gRenderer);
            HIGH_SCORE_7.render(gRenderer,855, 405,NULL);
            HIGH_SCORE_8.loadFromRenderedText(trans(_high_score[8]),gFont, RED_COLOR, gRenderer);
            HIGH_SCORE_8.render(gRenderer,855, 444, NULL);
            HIGH_SCORE_9.loadFromRenderedText(trans(_high_score[9]),gFont, RED_COLOR, gRenderer);
            HIGH_SCORE_9.render(gRenderer,855, 473,NULL);
            SDL_RenderPresent(gRenderer);
                ...
                bool &is_menu,bool &is_instruction,bool &high_sc)
{
    ...
    while(quit == false )
    {
                        ...
                        case SDLK_UP:
                        {
                            ...
                            while(player.speed_y > 0 && is_play == true)
                            {
                                ...
                                if( pause == false )
                                {
                                    ...
                                    score++;
                                    gScoreTexture.loadFromRenderedText(trans(score),gFont, WHITE_COLOR, gRenderer);
                                    gScoreTexture.render(gRenderer,1100, 10);
                                    rng(k,enemy);
                                    player.animation(6);
                                    SDL_Delay(50);
                                ...
                                {
                                    Mix_PauseMusic();
                                    gBackgroundPause.render(gRenderer,0,0,NULL);
                                     gScoreTexture.loadFromRenderedText(trans(score),gFont, WHITE_COLOR, gRenderer);
                                    gScoreTexture.render(gRenderer,1100, 10);
                                    gSpriteSheetTexture[player.index].render(gRenderer,player.x, player.y, &player.current_clip);
                                    gSpriteSheetTexture[k.index].render(gRenderer,k.x, k.y, &k.current_clip);
                                    SDL_RenderPresent(gRenderer);
                                }
                            }
        ...
        {
            play_again.render(gRenderer,0,0,NULL);
            gScoreTexture.loadFromRenderedText(trans(score),gFont, RED_COLOR, gRenderer);
            gScoreTexture.render(gRenderer,795, 340);
            SDL_RenderPresent(gRenderer);
            while( SDL_PollEvent( &e ) != 0 )
            {
